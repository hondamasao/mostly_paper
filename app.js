...
function renderGallery() {
  ...
  if (!grid) return;
  const filtered = PAINTINGS.filter(p => p.gallery && (activeGalleryCategory === 'all' || p.category === activeGalleryCategory));
  grid.innerHTML = '';
  filtered.forEach((p, i) => {
    const div = document.createElement('div');
    div.className = 'gallery-item' + (p.span2 ? ' span2' : '');
    div.innerHTML = `
      <img src="${p.image}" alt="${p.title}" loading="${i < 3 ? 'eager' : 'lazy'}" onerror="this.style.display='none'">
...
function renderPrintsGrid() {
  ...
  if (!grid) return;
  const filtered = PAINTINGS.filter(p => activeShopCategory === 'all' || p.category === activeShopCategory);
  grid.innerHTML = '';
  filtered.forEach(p => {
    ...
    card.innerHTML = `
      <div class="product-img-wrap">
        <img src="${p.image}" alt="${p.title}" loading="lazy" onerror="this.parentElement.style.background='#111'">
        <span class="product-badge">${p.category}</span>
      </div>
      <div class="product-info">
        <p class="product-name">${p.title} — Print</p>
        <p class="product-detail">${p.medium} · from ${fmt(minPrice)}</p>
    ...
    `;
    card.querySelector('.product-cta').addEventListener('click', e => { e.stopPropagation(); openSizeModal(p); });
...
function renderOriginalsGrid() {
  ...
  filtered.forEach(p => {
    ...
    card.innerHTML = `
      <div class="product-img-wrap">
        <img src="${p.image}" alt="${p.title}" loading="lazy" onerror="this.parentElement.style.background='#111'">
        <span class="product-badge ${avail ? '' : 'sold'}">${avail ? p.category : 'Sold'}</span>
      </div>
      <div class="product-info">
        ...
        <div class="product-bottom">
          ...
          <button class="product-cta" ${avail ? '' : 'disabled style="opacity:0.3;cursor:not-allowed"'}>${avail ? 'Add to cart' : 'Unavailable'}</button>
        </div>
      </div>
    `;
    if (avail) {
      card.querySelector('.product-cta').addEventListener('click', e => {
        e.stopPropagation();
        addToCart({ id: p.id + '-original', paintingId: p.id, title: p.title, type: 'original', size: p.original.size, price: p.original.price, image: p.image });
      });
    }
    grid.appendChild(card);
  });
...
function initGalleryTabs() {
  ...
  container.querySelectorAll('.gallery-tab').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      activeGalleryCategory = btn.dataset.cat;
      renderGallery();
    });
  });
}
...
function openSizeModal(painting) {
  activeSizeModal = { painting, selectedSize: painting.prints[1] };
...
function selectSize(index, painting) {
  ...
  document.querySelectorAll('.size-btn').forEach((b, i) => b.classList.toggle('selected', i === index));
  $('modal-price').textContent = fmt(activeSizeModal.selectedSize.price);
}
...
$('modal-add-btn').addEventListener('click', () => {
  ...
  addToCart({ id: painting.id + '-print-' + selectedSize.label.replace(/[^a-z0-9]/gi, ''), paintingId: painting.id, title: painting.title, type: 'print', size: selectedSize.label, price: selectedSize.price, printfulVariantId: selectedSize.printfulVariantId, image: painting.image });
  closeSizeModal();
});
...
function renderCart() {
  const items = $('cart-items'), footer = $('cart-footer'), count = $('cart-count');
  const total = cart.reduce((s, c) => s + c.price * c.qty, 0);
  const totalQty = cart.reduce((s, c) => s + c.qty, 0);
  count.textContent = totalQty;
  $('cart-total').textContent = fmt(total);
  if (cart.length === 0) { items.innerHTML = '<p class="cart-empty">Your cart is empty.</p>'; footer.style.display = 'none'; return; }
  footer.style.display = 'block';
  items.innerHTML = cart.map(item => `
    <div class="cart-item">
      <img class="cart-item-img" src="${item.image}" alt="${item.title}" onerror="this.style.background='#1a1a1a';this.removeAttribute('src')">
      <div>
  ...
  `).join('');
  items.querySelectorAll('.cart-item-remove').forEach(btn => btn.addEventListener('click', () => removeFromCart(btn.dataset.id)));
}

function openCart() { $('cart-drawer').classList.add('open'); $('cart-overlay').classList.add('open'); document.body.style.overflow = 'hidden'; }
function closeCart() { $('cart-drawer').classList.remove('open'); $('cart-overlay').classList.remove('open'); document.body.style.overflow = ''; }
...
$('checkout-btn').addEventListener('click', async () => {
  ...
  btn.disabled = true;
  try {
    const res = await fetch('/api/create-checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ items: cart }) });
    ...
    btn.disabled = false;
    setTimeout(() => { btn.textContent = 'Proceed to Checkout'; }, 3000);
  }
});
...
window.addEventListener('scroll', () => {
  const nav = $('nav');
  nav.style.borderBottomColor = window.scrollY > 60 ? 'rgba(122,21,21,0.35)' : 'rgba(122,21,21,0.2)';
}, { passive: true });

initGalleryTabs();
initShopCategoryTabs();
renderGallery();
renderShop();
renderCart();
